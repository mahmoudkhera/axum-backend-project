...
use crate::{
    ...
    db_handler::UserExt,
    dtos::{
        ForgotPasswordRequest, LoginUser, RegisterUser, ResetPasswordRequest, Response,
        UserLoginResponse, VerifyEmailQuery,
    },
    errors::{ErrorMessage, HttpError},
    mail::{self, mails_type::send_verification_email},
...
pub async fn verify_email(
    ...
    let result = app_state
        .db_client
        .get_user_by_token(&query_params.token)
        .await
    ...
    mail::mails_type::send_welcome_email(&user.email, &user.name)
        .await
        .unwrap();
...
pub async fn login(
    ...
    let password_matched = password::verify_password(&body.password, &user.password)
    ...
    if password_matched {
        ...
        let response = axum::response::Json(UserLoginResponse {
...
pub async fn forgot_password(
    ...
    let email_sent =
        mail::mails_type::send_forgot_password_email(&user.email, &reset_link, &user.name).await;
